    ...
    if (this.configService.get<Database>('DATABASE').PROVIDER === 'mysql') {
      ...
      results = await this.prismaRepository.$queryRaw`
        WITH rankedMessages AS (
          ...
          FROM Message
          LEFT JOIN Contact
            ON Contact.remoteJid = JSON_UNQUOTE(JSON_EXTRACT(Message.\`key\`, '$.remoteJid')) AND Contact.instanceId = Message.instanceId
          LEFT JOIN Chat
            ON Chat.remoteJid = JSON_UNQUOTE(JSON_EXTRACT(Message.\`key\`, '$.remoteJid')) AND Chat.instanceId = Message.instanceId
          ...
          ${timestampFilter}
          GROUP BY JSON_UNQUOTE(JSON_EXTRACT(Message.\`key\`, '$.remoteJid'))
          ORDER BY JSON_EXTRACT(Message.\`key\`, '$.remoteJid'), Message.messageTimestamp DESC
        )
